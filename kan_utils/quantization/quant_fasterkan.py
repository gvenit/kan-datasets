...
def _infer_rbf_mode(rsf_auto) -> str:
    ...
    RSFAuto.rbf is a RadialBasisFunction; that in turn stores .mode only for
    tanh2/gaussian/sample. For RSWAFF/PReLU/torch.nn modes, infer from class name."""
    rbf = rsf_auto.rbf  # RadialBasisFunction
    if hasattr(rbf, 'mode'):
        if '\n' in rbf.mode:
            return rbf.rbf
        return rbf.mode
    cls_name = type(rbf.rbf).__name__
    # Map internal class names back to the mode strings RadialBasisFunction accepts
    _cls_to_mode = {'RSWAFF': 'RSWAFF', 'PReLUGlobalParam': 'PReLU'}
    return _cls_to_mode.get(cls_name, cls_name)
...
def extract_range(tensor: torch.Tensor, bit_width: int, unsigned: bool = False) -> int:
    """
    Calculates the number of fractional bits available based on the tensor's value range.
    Args:
    ...
    Returns:
        int: Number of fractional bits for the quantized representation.
    """
...
class FixedPointFasterKANLayer(nn.Module):   
    ...
    def __init__(self, dtype_dict: dict = default_dtype, frac_bits_dict: dict = default_frac_bits, hardtanh: bool = False, collect_stats: bool = False, **kwargs):
        ...
        Args:
            ...
            hardtanh (bool, optional): Whether to use hardtanh activation. Defaults to False (and only applies to RSWAFF mode).
            ...
            **kwargs: Additional keyword arguments. If 'fasterKANLayer' is provided, initializes from its parameters;
                otherwise, expects 'input_dim', 'output_dim', 'num_grids', and optionally 'mode' for layer initialization.
        """
        ...
        if 'fasterKANLayer' in kwargs.keys():
            ...
            # Override rbf_func with a copy of the source layer's trained activation.
            # Needed for modes with learnable parameters (e.g. PReLU), harmless for others.
            self.rbf_func = copy.deepcopy(fasterKANLayer.rbf.rbf)
            # Copy rbf_norm if the source layer uses it (normalize_rbf=True)
            if getattr(fasterKANLayer, 'normalize_rbf', False):
                self.rbf_norm = fasterKANLayer.rbf_norm
        else :
    ...
    def quantize(self, fasterKANLayer: FasterKANLayer):
        ...
        # Keep rbf_func in sync for modes with learnable parameters (e.g. PReLU)
        self.rbf_func = copy.deepcopy(fasterKANLayer.rbf.rbf)
    ...
    def _load_from_state_dict(        
        ...
        # Re-create parameters to match the saved tensor dtype before copying.
        # Without this, if the module was constructed with a different dtype (e.g. default int8)
        # than what was saved (e.g. int16), super()._load_from_state_dict would silently
        # truncate the saved values when copying into the existing int8 buffers.
        for attr in ('grid', 'inv_denom', 'weight'):
            key = prefix + attr
            if key in state_dict:
                saved = state_dict[key]
                current = getattr(self, attr, None)
                if current is not None and current.dtype != saved.dtype:
                    setattr(self, attr, nn.Parameter(
                        torch.empty(saved.shape, dtype=saved.dtype),
                        requires_grad=False,
                    ))
    ...
    def __common_init(
        ...
        self.rbf_func = RadialBasisFunction(self.mode)
        self.rbf_norm = None
    ...
    def fit_frac_bits(self, x: torch.Tensor, fasterKANLayer: FasterKANLayer):
        ...
        with torch.no_grad():
            ...
            act = fasterKANLayer.rbf.rbf(sdff)
            ...
            act_for_linear = act.view(batch_size, -1)
            if getattr(fasterKANLayer, 'normalize_rbf', False):
                act_for_linear = fasterKANLayer.rbf_norm(act_for_linear)
            output = fasterKANLayer.linear(act_for_linear)
    ...
    def extra_repr(self) -> str:
        ...
        """
        return f"num_grids={len(self.grid)}"
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 3. Operation: activation function
        # hardtanh integer path is only valid for RSWAFF mode (sech² approximation)
...
class FixedPointFasterKAN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if model is not None:
            ...
            # Preserve normalization and residual connections from the source model
            if model.normalize:
            ...
            self.normalize = False
            self.residual = [False] * len(self.layers)
    ...
    def fit_frac_bits(self, x: torch.Tensor, fasterKAN: FasterKANLayer):
        ...
        output = x
        for i, (qlayer, fasterKANLayer) in enumerate(zip(self.layers, fasterKAN.layers)):
            # Mirror forward(): apply LayerNorm before each layer so that
            # fit_frac_bits sees normalized activations, not raw input values.
            if self.normalize:
                with torch.no_grad():
                    output = self.normalize[i](output.float())
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        for _iter, (layer, res) in enumerate(zip(self.layers, self.residual)):
            ...
            # by the layer's .to(int_dtype) cast, losing all fractional precision.
            x = quantize_fixed_point(x, layer.frac_bits_dict['grid'], *layer.dtype_dict['grid'])
            if res:
                x = layer(x) + x
            else:
                x = layer(x)
            # Dequantize back to float between layers so the next normalize sees the
            # correct value range (matching what fit_frac_bits calibrated on).
            if _iter < len(self.layers) - 1:
                x = dequantize_fixed_point(x, layer.frac_bits_dict['result'])
        return x
...
class FloatWrapperModule(nn.Module):
    ...
    def forward(self, x):
        # The model's forward now handles input quantization internally (after each normalize),
        # so we only need to dequantize the final integer output back to float.
