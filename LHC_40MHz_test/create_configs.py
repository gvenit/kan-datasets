...
from prepare_dataset import build_dataset, set_df_labels
from extract_statistics import get_corellate
# from custom_callbacks import UpdatableFloat

model_config = {}
df : pd.DataFrame = set_df_labels(build_dataset())
label_corr = get_corellate().loc['Label'].drop(['Label'])
...
train_config['callbacks']['train_iter_start'].append(
    object_to_config(
        'lambda *args, model=None, iteration=0, epoch=0, epochs=1, dataloader=None, **kwargs : model._modules["kan"].dropout_rate.set('
            f'{args.dropout} * torch.sigmoid( torch.tensor( ((epoch + (iteration / len(dataloader)) - 200) / 125) )).item()'
            # f'{args.dropout} / (1 + np.exp( ((epoch + (iteration / len(dataloader)) - 100) / epochs) ))'
        ')'
    )
)
