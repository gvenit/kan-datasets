...
def expand_df_labels(
    df: pd.DataFrame, 
    label_dict :dict = None,
    reduce_to_binary = True,
    ):
    if label_dict is None:
        label_dict = create_labels(df)
        
    for col, labels in label_dict.items():
        if (not reduce_to_binary) or len(labels.keys()) > 2:
...
def build_dataset(force = False):
    dataset_path = os.path.join(__dataset_dir,'Particle Physics Event Classification.csv')
    if force or not os.path.exists(dataset_path):
        ...
        kagglehub.whoami()
        kg.api.dataset_download_files("younusmohamed/particle-physics-event-classification-dataset", path=__dataset_dir, unzip=True, force=force)
        
    return get_dataset()

def get_dataset():
    dataset_path = os.path.join(__dataset_dir,'Particle Physics Event Classification.csv')
    df = pd.read_csv(dataset_path, index_col='EventId')
    df.replace(-999, np.nan, inplace=True)
    for col in df.dtypes[np.isin(df.dtypes.values,('object','str'))].index:
        if col == 'Date':
            continue
        idx = df[col].isna()
    ...
    for col in df.columns:
        if col in ['EventId', 'Weight', 'Label']:
            continue
        df[col] = df.groupby('PRI_jet_num')[col].transform(
