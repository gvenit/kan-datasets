...
class SkinCancerDataset (Dataset): 
    ...
    def __init__(
        ...
        input_img_dims  = None,
        path_cols       : dict[Literal['image','mask','volume'],list[str]] = {},
        ...
        if not hasattr(self,'_dummy_img'):
            keys = {
                id : self.df.loc[0, self.path_cols_dict[
                    ...
                    for id in self.__cv2_flags.keys()
            }
            self._dummy_img = {
                key : cv2.imread(
                    pth, 
    ...
    def __get_image(self, path_dict, num_idx):
        ...
        return ({
                key : val.astype(bool).astype('uint8')[...,None]
                        if self.path_type_dict[key] == 'mask' else val.astype('uint8')
...
if __name__ == '__main__':
    ...
    dataset = SkinCancerDataset(
        ...
        input_cols = ['Image'],
        output_cols= {
            'classifier'    : [col for col in df.columns if col.startswith('Lesion')],
            'segmentor'     : ['Mask'],
            'reconstructor' : ['Image'],
        },
        input_img_dims  = None,
    ...
    for data, target, *key in dataset:
        print(data.shape)
        for head, head_target in target.items():
            print(head, head_target.shape)
        print(key)
