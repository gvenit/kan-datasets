    ...
    object_to_config(
        torch.nn.Sequential, *[
            object_to_config(
                ...
                for in_features, out_features in zip(args.hidden_layers[:-1],args.hidden_layers[1:],)
        ],
        target_name       = 'model',
    )
)
...
def build_test_dir(args, train_config, model_config, top_dir = None, test_version = None):
    pdir = os.path.join(
        '_'.join([ str(_) for _ in args.hidden_layers]),
...
pdir = build_test_dir(args, train_config, model_config, top_dir=args.dest_top_dir, test_version=args.test_version)
if not args.export :
