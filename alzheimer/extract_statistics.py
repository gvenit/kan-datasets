...
from prepare_dataset import build_dataset, expand_df_labels, set_df_labels, normalize_dataset
...
def get_corellate():
    if 'df_corr' not in globals():
        ...
        if not os.path.exists(path):
            df_corr = extract_correlate(set_df_labels(build_dataset()))
            df_corr.to_csv(path)
        else:
...
if __name__ == '__main__':
    ...
    df = set_df_labels(build_dataset())
