...
model_config['Image Encoder Summary'] = str(get_summary(
    img_enc,
    next(iter(test_loader))[0]
))
model_config['Image Decoder Summary'] = str(get_summary(
    img_dec,
    img_enc(next(iter(test_loader))[0])
))
...
test_df = pd.read_csv(os.path.join(rslt_path,f'rslt.csv'), index_col='Index')
test_df.index = pd.Index(full_dataset.get_keys(test_df.index.to_list()))
test_df.to_csv(os.path.join(rslt_path,f'rslt.csv'))
