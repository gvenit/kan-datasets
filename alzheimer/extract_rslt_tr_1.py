...
gt_df = pd.read_csv(os.path.join(rslt_path, 'ground_truth.csv'))
pr_df = pd.read_csv(os.path.join(rslt_path,f'{args.epoch}.csv'))

for idx in gt_df.index:
    gt_img = gt_df.loc[idx].values
    pr_img = pr_df.loc[idx].values