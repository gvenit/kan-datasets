...
class MNISTDataset(Dataset):
    """
    Simple MNIST dataset wrapper for use with PyTorch DataLoader.
    ...
    def __getitems__(self, idx):
        ...
        if self.task == 'multiclass':
            ...
            label = torch.nn.functional.one_hot(label.long(), 10).float()

        if self.return_weights:
            # For MNIST, all samples have equal weight
            weight = torch.tensor(1.0, dtype=torch.float32)

        out = []
        for (_idx, _data, _label) in zip(idx, data, label):
...
class IterableMNISTDataset(IterableDataset):
    """
    Simple MNIST dataset wrapper for use with PyTorch DataLoader.
    Loads data using prepare_dataset.py's get_dataset() function.
    """
    def __init__(
        ...
        super(IterableMNISTDataset, self).__init__()
...
if __name__ == '__main__':
    ...
    print("Testing MNISTDataset...")
    data, labels = get_dataset('train_val')

    train_dataset = MNISTDataset(
        data, labels,
        preprocess_data=lambda images: normalize_data(images),
        flatten=True,
    )
    print(f"Train dataset size: {len(train_dataset)}")
    sample_data, sample_label = train_dataset[np.random.randint(0, len(train_dataset))]
    print(f"Data shape: {sample_data.shape}")
    print(f"Label: {sample_label}")
    print(f"Data dtype: {sample_data.dtype}")
    print(f"Label dtype: {sample_label.dtype}")
    print('Data min :', sample_data.min())
    print('Data max :', sample_data.max())

    fname = os.path.join(os.path.dirname(__file__), 'dataset', 'sample.png')
    print('Sample image :', fname)
    img = sample_data.reshape(28, 28).numpy()
