...
__train_file = "mnist_train.pkl"
__test_file = "mnist_test.pkl"
...
def build_dataset(force: bool = False) -> None:
    """
    Download MNIST (via torchvision) and save as pickle files.
    ...
    try:
        ...
        raise ImportError("torchvision is required to download MNIST. Install with: pip install torchvision")

    # Download train and test sets
    train_ds = datasets.MNIST(root=str(__dataset_dir), train=True, download=True)
    test_ds = datasets.MNIST(root=str(__dataset_dir), train=False, download=True)

    # Convert to numpy arrays; MNIST .data is a uint8 tensor of shape (N, 28, 28)
    train_data = train_ds.data.numpy()             # (60000, 28, 28)
    train_labels = np.array(train_ds.targets)      # (60000,)
    test_data = test_ds.data.numpy()               # (10000, 28, 28)
...
def get_class_names() -> List[str]:
    ...
    try:
        from torchvision import datasets
        datasets.MNIST(root=str(__dataset_dir), train=True, download=False)
...
def calculate_statistics(force: bool = False, save: bool = True) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute mean and std on the training set (pixel values in [0,1]).
    Returns (mean, std) as numpy scalars wrapped in arrays of shape (1,).
    """
    if not save:
        # Compute on the fly
...
def _compute_stats_from_train() -> Tuple[np.ndarray, np.ndarray]:
    ...
    data = data_dict["data"].astype(np.float32) / 255.0   # (N, 28, 28)
...
def get_dataset(
    ...
    """
    Load MNIST data for the requested subset.
...
def normalize_data(data: np.ndarray) -> np.ndarray:
    """
    Standardise image data using pre‑computed mean and std.
    Handles flattened (N, 784) and image‑shaped (N, 28, 28) inputs.
    """
    ...
    if data.ndim == 2 and data.shape[1] == 784:
        # Flattened: normalize and return as-is
