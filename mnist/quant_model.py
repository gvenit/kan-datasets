...
if __name__ == '__main__':
    ...
    print("\n ------ Quantization Script for MNIST Dataset ------ ")
    ...
    calib_data = None
    if args.fit_model:
        print("\n ---- Building calibration dataset for frac_bits fitting ...")
        first_model_cfg = load_config(runs[0]['model_cfg_path'])
        first_train_cfg = load_config(runs[0]['train_cfg_path'])

        # Mirror the same resize preprocessing used during training
    ...
    for run in runs:
        ...
        print(f'  >> Saved bin files to:\n     {models_dir}/extracted_params/ \n      {models_dir}/packetized_params/')
