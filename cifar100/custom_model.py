...
class CustomModel(torch.nn.Module):
    def __init__(
        ...
        *h_chns, h_rows, h_cols = [[_[_pos] for _ in hidden_shapes] for _pos in range(len(hidden_shapes[0]))]
        self.apply_to_rows = torch.nn.ModuleList(
            SubBatch(
                input_data_dim  = -1,
                output_data_dim = -1,
                model           = FasterKAN(
                    hidden_layers   = [_in, _out],
                    num_grids       = num_grids,
                    grid_min        = grid_min,
                    grid_max        = grid_max,
            ...
            for _in, _out in zip(h_cols[:-1], h_cols[1:])
        )
        self.apply_to_cols = torch.nn.ModuleList(
            SubBatch(
                input_data_dim  = -1,
                output_data_dim = -1,
                model           = FasterKAN(
            ...
            for _in, _out in zip(h_rows[:-1], h_rows[1:])
        )
        if len(h_chns) > 0:
            h_chns = h_chns[0]
            self.apply_to_chns = torch.nn.ModuleList(
                SubBatch(
                    input_data_dim  = -1,
                    output_data_dim = -1,
                    model           = FasterKAN(
                ...
                for _in, _out in zip(h_chns[:-1], h_chns[1:])
            )
        else :
            self.apply_to_chns = None
            
        flattened_size = torch.prod(torch.tensor(hidden_shapes[-1]))
        self.encoded = SubBatch(
            input_data_dim  = -3,
            output_data_dim = -1,
            model           = torch.nn.Sequential(OrderedDict(
                flatten  = torch.nn.Flatten(),
                head     = FasterKAN(
                    hidden_layers   = [flattened_size, *encoded_state_size],
                    num_grids       = num_grids,
                    grid_min        = grid_min,
                    grid_max        = grid_max,
                    inv_denominator = inv_denominator,
    ...
    def forward(self, x:torch.Tensor):
        for _iter in range(len(self.apply_to_rows)):
            x = self.apply_to_rows[_iter](x)
            x = x.transpose(-1, -2)
            x = self.apply_to_cols[_iter](x)
            if self.apply_to_chns is not None:
                x = x.transpose(-1, -3)
                x = self.apply_to_chns[_iter](x)
                x = x.moveaxis(-1, -3)
            else :
                x = x.transpose(-1, -2)
...
class ImageKANEncoder(torch.nn.Module):
    ...
    :param reduce_dim: If specified, the dimension to apply reduction (of the input tensor).
    :type reduce_dim: int, list[int], Optional
    :param reduction: If `reduce_dim` is specified, the reduction strategy.
    ...
    def __init__(
        ...
        self.reduce_dim = reduce_dim if reduce_dim > 0 else reduce_dim+1,
    ...
    def forward(self, x):
        ...
        if self.reduce_dim is not None :
            if self.reduction == 'sum':
                x = x.sum(dim=self.reduce_dim)
            elif self.reduction == 'mean':
                x = x.mean(dim=self.reduce_dim)
            elif self.reduction == 'none':
                pass
            else :
                raise NotImplementedError(f'Reduction strategy {self.reduction} is not implemented.')
        return x
    
class SymmetricImageKANEncoder(ImageKANEncoder):
    '''Symmetric Image encoder model using FasterKAN layers.
    ...
    def __init__(
        ...
        assert input_shape[0] == input_shape[1], f'Asymmetric input shape: {input_shape}'
        super(SymmetricImageKANEncoder, self).__init__(
    ...
    def forward(self, x: torch.Tensor):
        x = super().forward(x) + super().forward(x.transpose(-2,-1))
        return x
    
class AsymmetricImageKANEncoder(torch.nn.Module):
    '''Asymmetric Image encoder model using FasterKAN layers.
    ...
    def __init__(
        ...
        super(AsymmetricImageKANEncoder, self).__init__()
        self.model = ImageKANEncoder(
            input_shape         = input_shape.copy(),
            hidden_state_size   = hidden_state_size.copy(),
            encoded_state_size  = encoded_state_size.copy(),
            num_grids           = num_grids,
            grid_min            = grid_min,
            grid_max            = grid_max,
            inv_denominator     = inv_denominator,
    ...
    def forward(self, x: torch.Tensor):
        x = self.model(x) + self.model_t(x.transpose(-2,-1))
        return x
